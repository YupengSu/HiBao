...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActiontestData,
                            payload->reportAction.testData);
    ret = cJSON_PrintUnformatted(root);
    cJSON_Delete(state);
...
#define CN_PROFILE_TOPICFMT_TOPIC            "$shadow/operation/4FFMYT2AVB/Hi3861_mqtt"
